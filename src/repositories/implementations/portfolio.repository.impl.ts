...
export const createPortfolioRepository = (): PortfolioRepository => {
  ...
  return {
    ...
    getPortfolioByUserId: async (userId) => {
      const portfolios = await createPortfolioRepository().getAllPortfolios(
        userId,
      );
    ...
    getItemsByCategory: async (categoryId, portfolioId) => {
      const [rows] = await db.execute<RowDataPacket[]>(
        `SELECT 
           alloc.item_id, 
           alloc.category_id, 
         ...
         FROM item_allocation alloc 
         JOIN items i ON i.id = alloc.item_id 
         WHERE (? IS NULL OR alloc.portfolio_id = ?) AND alloc.category_id = ?`,
        [portfolioId ?? null, portfolioId ?? null, categoryId],
      );
    ...
    addItem: async (categoryId, itemId, customInfo) => {
      const portfolioId = categoryId;
      const portion = 0;
      const conn = await db.getConnection();
      try {
        await conn.beginTransaction();
        ...
        await rebalancePortions(conn, portfolioId, itemId ?? null, portion);
        await recalculatePortfolioMetrics(conn, portfolioId);
        await conn.commit();
      } catch (e) {
        await conn.rollback();
    ...
    updateItemRelativePortions: async (categoryId, portions, portfolioId) => {
      const targetPortfolioId = portfolioId ?? 0;
      const conn = await db.getConnection();
      try {
        await conn.beginTransaction();
        ...
        for (const p of portions) {
          ...
          await conn.execute(
            ...
            [absolutePortion, targetPortfolioId, categoryId, itemId],
          );
        }
        await recalculatePortfolioMetrics(conn, targetPortfolioId);
        await conn.commit();
      } catch (e) {
        await conn.rollback();
        throw e;
    ...
    updateItemInfo: async (itemId, itemInfo, portfolioId) => {
      const updates = [];
      const values = [];
      ...
      if (updates.length === 0) return;
      values.push(portfolioId ?? 0, itemId);
      await db.execute(
    ...
    getAvailableCategories: async (portfolioId, userId = 0) => {
      const [rows] = await db.execute<RowDataPacket[]>(
    ...
    getAvailableItems: async (categoryId, userId = 0) => {
      const [rows] = await db.execute<RowDataPacket[]>(
